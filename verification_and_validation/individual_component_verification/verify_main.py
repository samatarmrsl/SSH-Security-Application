...
def test_status_persists_configured_mode(tmp_path, capsys) -> None:
    ...
    assert result == 0
    assert "active mode" in capsys.readouterr().out
    repositories = RepositorySet(Database(tmp_path / "cli.db"))
...
def test_full_fixture_detection_command(tmp_path, capsys) -> None:
    ...
    assert "192.168.56.40" in table
    assert "High Risk" in table
    assert "80" in table
...
def test_invalid_unblock_is_reported(tmp_path, capsys) -> None:
    config = write_local_config(tmp_path)
...
def test_monitor_command_builds_unprivileged_pipeline_and_stops(
    tmp_path,
    monkeypatch,
) -> None:
    config = write_local_config(tmp_path)
    ...
    monkeypatch.setattr(
        "ssh_security_application.main.ApplicationController",
        FakeController,
    )

    result = main(["--config", str(config), "monitor"])
