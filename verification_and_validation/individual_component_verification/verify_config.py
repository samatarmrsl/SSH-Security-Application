...
def test_packaged_defaults_match_committed_defaults() -> None:
    ...
    committed = json.loads(
        (
            repository_root / "application_configuration" / "safe_default_configuration.json"
        ).read_text(encoding="utf-8")
    )
    packaged = json.loads(DEFAULT_CONFIG_PATH.read_text(encoding="utf-8"))

    assert DEFAULT_CONFIG_PATH.name == "default_config.json"
    assert packaged == committed
    assert committed["response"]["mode"] == "simulation"
    assert "dashboard" not in committed
...
def test_default_configuration_is_safe() -> None:
    ...
    assert not hasattr(settings, "dashboard")
