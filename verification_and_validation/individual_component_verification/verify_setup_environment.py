...
from ssh_security_application.setup_environment import (
    build_local_config,
    discover_network,
    main,
    write_local_config,
...
def test_preview_lists_plan_without_privileged_commands(capsys, monkeypatch) -> None:
    monkeypatch.setattr(
        "ssh_security_application.setup_environment.shutil.which",
...
def test_apply_runs_guarded_setup_and_writes_valid_config(
    ...
    fake_module = (
        ...
        / "ssh_security_application"
        / "setup_test_environment.py"
    )
    monkeypatch.setattr(
        "ssh_security_application.setup_environment.__file__",
        str(fake_module),
    )
    monkeypatch.setattr(
        "ssh_security_application.setup_environment.shutil.which",
