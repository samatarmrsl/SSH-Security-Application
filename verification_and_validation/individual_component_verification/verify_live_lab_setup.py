...
from ssh_security_application.live_lab_setup import (
    verify_installation as run_installation_verification,
)
...
def test_plan_autodetects_server_and_firewalld_zone(monkeypatch) -> None:
    monkeypatch.setattr(
        "ssh_security_application.live_lab_setup.shutil.which",
        lambda name: f"/usr/bin/{name}",
    )
    monkeypatch.setattr(
        "ssh_security_application.live_lab_setup._ufw_is_enabled",
        lambda: False,
    )
...
def test_live_config_enables_two_minute_automatic_response() -> None:
    ...
    assert "dashboard" not in document
...
def test_preview_performs_no_privileged_commands(capsys, monkeypatch) -> None:
    ...
    assert "SSH endpoint: 192.168.12.1:22" in output
...
def test_firewall_service_initializes_and_cleans_dedicated_chain() -> None:
    unit = (
        REPOSITORY_ROOT / "installation_and_service_setup/ssh-security-application-firewall.service"
    ).read_text(encoding="utf-8")
    application_unit = (
        REPOSITORY_ROOT / "installation_and_service_setup/ssh-security-application.service"
    ).read_text(encoding="utf-8")
    ...
    assert "After=network-online.target ssh.service ssh-security-application-firewall.service" in (
        application_unit
    )
...
def test_apply_plan_orchestrates_complete_idempotent_install(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "ssh_security_application.live_lab_setup._bootstrap_tools",
        lambda: {"sudo": "/tool/sudo"},
    )
    monkeypatch.setattr(
        "ssh_security_application.live_lab_setup._installed_tools",
        lambda: tools,
    )
    monkeypatch.setattr(
        "ssh_security_application.live_lab_setup._required_executable",
    ...
    enabled = [
        ...
        and command[-1].startswith("ssh-security-application")
    ]
    assert enabled == [
...
def test_verify_installation_checks_services_chain_client_and_endpoints(
    ...
    monkeypatch.setattr(
        "ssh_security_application.live_lab_setup._tcp_listener_check",
        lambda _host, _port: (True, "listener ready"),
    )

    run_installation_verification(plan, runner=runner, use_sudo=True)

    output = capsys.readouterr().out
    assert "[PASS] ssh-security-application-firewall.service" in output
    assert "[PASS] project firewall chain" in output
    assert "[PASS] disposable client baseline: not blocked" in output
    assert "[PASS] SSH listener" in output
