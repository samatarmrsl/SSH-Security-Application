...
DATABASE_PATH = DATA_DIRECTORY / "ssh_security_app.db"
LOG_PATH = LOG_DIRECTORY / "ssh_security_app.log"
FIREWALL_UNIT = "ssh-security-app-firewall.service"
APPLICATION_UNIT = "ssh-security-app.service"
DASHBOARD_UNIT = "ssh-security-app-dashboard.service"
...
class LiveLabPlan:
    ...
    def dashboard_url(self) -> str:
        return f"http://{self.server_ip}:{self.dashboard_port}"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--dashboard-port", type=_port, default=8501)
    parser.add_argument(
...
def verify_installation(
    ...
    *,
    ...
    if plan.firewall_frontend == "firewalld":
        ...
        if plan.firewalld_zone is None:
            ...
            frontend_ok = all(
                ...
                for port in (plan.ssh_port, plan.dashboard_port)
            )
            frontend_detail = (
                f"firewalld zone {plan.firewalld_zone}; "
                f"{plan.client_ip} -> {plan.server_ip} ports "
                f"{plan.ssh_port},{plan.dashboard_port}"
            )
    ...
    dashboard_ok, dashboard_detail = _wait_for_dashboard(plan.dashboard_url)
    checks.append(("dashboard HTTP API", dashboard_ok, dashboard_detail))
...
def _install_systemd_units(
    ...
    *,
    ...
    for name in unit_names:
        source = plan.repository_root / "systemd" / name
    ...
    _checked_run(
        [
            ...
            "-m",
            "0644",
            str(plan.repository_root / "systemd/ssh-security-app-tmpfiles.conf"),
...
def _start_services(
    *,
    ...
    for unit in (FIREWALL_UNIT, APPLICATION_UNIT, DASHBOARD_UNIT):
        _checked_run(
...
def _stop_existing_services(
    *,
    ...
    for unit in (DASHBOARD_UNIT, APPLICATION_UNIT, FIREWALL_UNIT):
        _captured_run(
...
def _validate_repository(root: Path) -> None:
    required = (
        root / "pyproject.toml",
        root / "src/ssh_security_app/default_config.json",
        root / "systemd/ssh-security-app-firewall.service",
        root / "systemd/ssh-security-app.service",
        root / "systemd/ssh-security-app-dashboard.service",
        root / "systemd/ssh-security-app-tmpfiles.conf",
    )
...
def _wait_for_dashboard(url: str, *, timeout_seconds: float = 10.0) -> tuple[bool, str]:
        ...
        try:
            with urllib.request.urlopen(snapshot_url, timeout=2) as response:
                if response.status == 200:
                    ...
                    last_error = "snapshot response was incomplete"
