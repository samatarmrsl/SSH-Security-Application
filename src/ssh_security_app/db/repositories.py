...
class ActionRequestRepository:
    ...
    def get(self, request_id: str) -> ActionRequest | None:
        with self.database.connection() as connection:
            row = connection.execute(
                "SELECT * FROM action_requests WHERE request_id = ?",
                (request_id,),
            ).fetchone()
        return _action_request_from_row(row) if row else None

    def list_recent(self, limit: int = 100) -> list[ActionRequest]:
        with self.database.connection() as connection:
            rows = connection.execute(
                """
    ...
    def has_pending_for_block(self, block_id: str) -> bool:
        with self.database.connection() as connection:
            row = connection.execute(
                """
                ...
                LIMIT 1
                """,
                (block_id, ActionRequestStatus.PENDING.value),
            ).fetchone()
        return row is not None
