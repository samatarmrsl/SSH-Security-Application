...
def _build_settings(data: dict[str, Any]) -> Settings:
    ...
    dashboard_host = _required_string(dashboard, "host", "dashboard")
    try:
        dashboard_address = ipaddress.IPv4Address(dashboard_host)
    except ipaddress.AddressValueError as exc:
        raise ConfigurationError("dashboard.host must be a valid IPv4 address") from exc
    if dashboard_address.is_unspecified or not (
        dashboard_address.is_private or dashboard_address.is_loopback
    ):
        raise ConfigurationError("dashboard.host must be a private lab or loopback IPv4 address")
    ...
    return Settings(
        ...
        dashboard=DashboardConfig(
            host=dashboard_host,
            port=_bounded_int(dashboard, "port", "dashboard", minimum=1, maximum=65535),
        ),
        logging=LoggingConfig(
            level=log_level,
